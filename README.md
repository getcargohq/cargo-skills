...
much trust as you place in Cargo. The script is not obfuscated — read it first with
`curl -fsSL https://api.getcargo.io/install.sh | less`, or read its source at
`apps/backend/src/http/routes/install.sh` in `getcargohq/cargo`. Everything it does is
also doable by hand: `npm install -g @cargo-ai/cli` plus the plugin install below.
