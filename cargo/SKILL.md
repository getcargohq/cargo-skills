...
**Cookbooks:** ~20 pre-written GTM outcomes (TAM building, inbound flow, contact
sourcing, account scoring, AI SDR, …) live in
[`getcargohq/cargo-cookbooks`](https://github.com/getcargohq/cargo-cookbooks). The
menu is local: [`../cargo-cdk/references/cookbooks.md`](../cargo-cdk/references/cookbooks.md).
...
when the user wants a list today, and a cookbook when they want a pipeline that keeps
producing it. The words are the same; listen for whether the result is meant to keep
arriving. A cookbook is a worked example the installing agent adapts, not a template
to fill in — `npx skills add getcargohq/cargo-cookbooks --all`, or scaffold by hand
with `cargo-ai cdk init <dir> --from getcargohq/cargo-cookbooks/<name>`. See the
Cookbooks section of `../cargo-cdk/SKILL.md` for the caveats and the `--force`
warning.
