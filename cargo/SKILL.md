...
- **Prefer built-in actions + expressions when building a node graph.** Avoid `python`, `script` (JS), and raw HTTP nodes unless necessary: use `variables` for transforms, the native `agent` node for LLM calls, the integration's dedicated connector action for APIs, and `branch`/`filter`/`switch` for routing. Write to a Cargo model with native `modelUpsert`, never an HTTP call to its `/records/ingest` webhook, and never add a `script` node just to build the next node's input — every input, HTTP `bodyJson` included, takes expressions. See the hard rules in `../cargo-orchestration/references/node-selection.md`.
