...
- **Use a code node only when you need one.** A template expression is inline JavaScript, so a small transformation goes in `{{ }}` in the field that needs it, not in a `python`/`script` node. Prefer native actions too: `variables` for shared values, the `agent` node for LLM calls, connector actions for APIs, `modelUpsert` to write a Cargo model, and `branch`/`filter`/`switch` for routing. See `../cargo-orchestration/references/node-selection.md`.
