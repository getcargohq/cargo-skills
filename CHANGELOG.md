...
### `cargo-ai` → 2.3.0, `cargo-orchestration` → 1.9.0, `cargo-gtm` → 1.17.0, `cargo-connection` → 1.3.1, `cargo-observability` → 1.0.2 — `orchestration action list`, and `config` stopped shouting

Tracks the platform-MCP branch in the CLI/backend repo (`cursor/platform-mcp-server-55ed`): a new `orchestration action list` command, and `config` split off the top-level action type.

- **`cargo-ai mcp` with no `--server` now means something different.** It used to resolve "the workspace's only MCP server" and fail with `InvalidUsage` when there were none or several; it now bridges the platform `/mcp` (still after `CARGO_MCP_SERVER_UUID`). Same command, different server, depending on CLI version — so it is written down in [`cargo/references/gotchas.md`](cargo/references/gotchas.md) rather than left to be discovered.

- **New: `cargo-ai orchestration action list <keywords>`.** Free, executes nothing, and searches the integration catalog, Cargo native actions, workspace tools, and agents in one call — returning a ready-to-paste `action` object (with `connectorUuid` already resolved), the action's **credit costs**, and its autocomplete slugs. Documented in [`cargo-orchestration`](cargo-orchestration/SKILL.md) (decision tree, discovery list, quick reference) and in full — flags, ranking, response, the excluded structural natives — in [`references/examples/actions.md`](cargo-orchestration/references/examples/actions.md). `cargo-gtm` points at it as a slug-and-price sanity check that does **not** replace the provider playbook.

- **`config` on a top-level action is now optional — and misusing it fails silently.** `Action` and `ConfiguredAction` split apart: `execute`/`execute-batch` accept an action with no `config` key, while workflow **nodes**, alert `--actions`, play `healthAlertActions`, and `get-output-schema` still require it. The dangerous half is the removed guard: putting an action's *inputs* in `config` used to fail loudly with `A top-level action does not use action.config…`, and now they are dropped on the way in and the action runs with **no inputs** — a provider-side missing-field error, or an empty result, that never mentions `config`. Recorded in the router's critical rules, `cargo-connection`'s input-schema section, and the gotchas table. `"config": {}` stays valid on every version, so the ~289 existing examples were left alone deliberately.
