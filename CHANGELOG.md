...
- **`--is-finished` dropped from the six snippets that taught it** — `SKILL.md`, `exports.md`, `run-analytics.md`, `troubleshooting.md` — in favor of `--statuses success,error`, which expresses the same filter and is correct either way. The flag currently returns `400 unrecognized_keys: isFinished` on all four run commands; the fix is in `getcargohq/cargo#5579` and lands with the **backend deploy**, not with a CLI release, so these docs deliberately say nothing about the flag's state rather than carrying a caveat that expires on a date the bundle can't see. `troubleshooting.md` gains a generic `400 unrecognized_keys` row instead, which stays useful whatever the specific flag.
