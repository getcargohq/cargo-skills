 ...
 *
 *   node .github/scripts/sync-cdk-examples.ts            # regenerate the reference
 *   node .github/scripts/sync-cdk-examples.ts --check    # CI: fail if stale
 *   node .github/scripts/sync-cdk-examples.ts --refresh  # re-pull from ../gtm-skills
...
const snapshotPath = join(repoRoot, ".github/data/cdk-examples.json");
const referencePath = join(repoRoot, "cargo-cdk/references/cdk-examples.md");
...
async function refresh(): Promise<CdkExample[]> {
  ...
  const out: CdkExample[] = [];
  for (const s of catalog.skills) {
    if (s.kind !== "cdk-example") continue;
    out.push({
      slug: s.name, kind: "cdk-example", outcome: s.job, state: s.state ?? "to-be-approved",
  ...
  for (const p of catalog.pending ?? []) {
    out.push({ slug: p.name, kind: "cdk-example", outcome: p.job, state: p.state ?? "to-be-approved", chain: null, requires: [], hasSkill: false, variations: [] });
  }
  return out.sort((a, b) => a.slug.localeCompare(b.slug));
}

function render(examples: CdkExample[]): string {
  const withSkill = examples.filter((b) => b.hasSkill);
  const L: string[] = [];
  L.push("<!-- Generated by .github/scripts/sync-cdk-examples.ts from gtm-skills' catalog.json. Do not edit by hand. -->");
  L.push("");
...
const examples: CdkExample[] = process.argv.includes("--refresh")
  ? await refresh()
  : JSON.parse(readFileSync(snapshotPath, "utf8"));

if (process.argv.includes("--refresh")) {
  writeFileSync(snapshotPath, JSON.stringify(examples, null, 2) + "\n");
  console.log(`snapshot: ${examples.length} CDK examples -> ${snapshotPath}`);
}
...
if (process.argv.includes("--check")) {
  ...
  if (current !== rendered) {
    console.error(
      "cargo-cdk/references/cdk-examples.md is stale.\n  Run: node .github/scripts/sync-cdk-examples.ts",
    );
    process.exit(1);
  }
  console.log(
    `ok: CDK-example menu matches the snapshot (${examples.length} CDK examples)`,
  );
} else {
  writeFileSync(referencePath, rendered);
  console.log(`wrote ${referencePath} (${examples.length} CDK examples)`);
}
