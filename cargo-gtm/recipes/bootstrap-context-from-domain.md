# Recipe — Bootstrap workspace context from a domain

Use this skill when the user wants to **seed an empty (or thin) context repo from public data**, starting from nothing more than their company's domain. The skill enriches the company via cargo native + waterfall + theirStack, scrapes public sources in parallel sub-agents, and writes one file per atomic concept into the workspace's git-backed context repo via `cargo-ai context runtime write` — skipping any domain that already has content.

Output: a populated `global/`, `icp/`, `persona/`, `client/`, `proof/`, `signal/` (and where evidence supports it, `alternative/`, `objection/`, `insight/`) — enough that a fresh agent session can hold a coherent conversation about the company.
...
Most cargo-gtm recipes are *outbound* (find / enrich / sync prospects). Bootstrap is *inbound*: it populates the workspace's own self-knowledge so every downstream agent, play, and tool has typed prose to reason from. It is the **automatable Phase 1** of the broader lifecycle documented in [`../../cargo-context/references/examples/bootstrap-and-update.md`](../../cargo-context/references/examples/bootstrap-and-update.md). Phase 2 (call-driven refinement) is deliberately out of scope here — see the "What this recipe does NOT do" section.
...
For domains that exist but are thin (1 entry), still write *new* files into them, but never `runtime edit` an existing file in bootstrap mode. Edits are for the refresh phase (see [Phase 2](../../cargo-context/references/examples/bootstrap-and-update.md#phase-2--refresh-from-real-calls)), not bootstrap.
...
- **No refinement from sales calls.** That's [Phase 2 of `bootstrap-and-update.md`](../../cargo-context/references/examples/bootstrap-and-update.md#phase-2--refresh-from-real-calls) — deliberately human-in-the-loop. Auto-promoting call-derived claims into context produces plausible-sounding but shallow ICPs.
...
- **No promotion past the repetition threshold.** See [authoring rules of thumb](../../cargo-context/references/conventions.md#authoring-rules-of-thumb). Bootstrap claims come from public sources, which count as one source — note the URL in the file body, don't promote to a confident assertion.
...
cargo-ai workspaceManagement report create \
  --title "bootstrap-context-from-domain: <one-line summary>" \
