...
**Purpose:** Follow-up line to a contact who just changed jobs, anchored on a first-90-days priority. **Variables:** {{first_name}}, {{new_title}}, {{new_company}}, {{previous_company}}, {{relationship_context}}. **Model guidance:** claude-haiku-4-5 for bulk; claude-sonnet-5 for named accounts. **Output:** one sentence, ≤35 words, plain text.
...
**Purpose:** Connection-request note that fits LinkedIn's hard character limit and doesn't pitch. **Variables:** {{first_name}}, {{title}}, {{company_name}}, {{reason_for_connecting}}. **Model guidance:** claude-haiku-4-5. **Output:** plain text, ≤300 characters (hard limit).
...
**Purpose:** Three deliberately different subject-line styles for the same email, ready for A/B rotation. **Variables:** {{first_line}}, {{signal_summary}}, {{company_name}}. **Model guidance:** claude-haiku-4-5. **Output:** JSON array of exactly 3 strings.
...
**Purpose:** First line to a stale contact where a fresh signal — not "checking in" — is the reason for writing. **Variables:** {{first_name}}, {{last_touch_summary}}, {{months_since_contact}}, {{fresh_signal}}. **Model guidance:** claude-haiku-4-5. **Output:** one sentence, ≤35 words — or `NULL` when there is no fresh signal.
