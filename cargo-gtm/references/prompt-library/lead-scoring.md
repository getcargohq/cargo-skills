...
Prompts that turn enriched records into scores, tiers, and disqualifications with explicit rubrics — so two runs on the same data agree. Run through `anthropic.instruct` with `temperature: 0` (0.2 max) (haiku tier only — on `claude-sonnet-5` omit `temperature` entirely: it rejects non-default sampling parameters); scoring wants determinism. All JSON outputs are parse-ready for downstream filter/branch nodes.

### icp-fit-score

**Purpose:** Score a company 1-10 against a written ICP definition with a fixed rubric. **Variables:** {{icp_description}}, {{company_name}}, {{company_summary}}, {{industry}}, {{employee_count}}, {{country}}. **Model guidance:** claude-haiku-4-5 for bulk sweeps; claude-sonnet-5 when the ICP has many soft, judgment-heavy criteria. **Output:** JSON `{score, rationale, missing_data}`.
...
**Purpose:** Score fit from detected technologies — complementary tools raise it, incumbents cap it. **Variables:** {{detected_technologies}}, {{complementary_technologies}}, {{competing_technologies}}. **Model guidance:** claude-haiku-4-5. **Output:** JSON `{score, complementary_matches, competing_matches, displacement_candidate, rationale}`.
...
**Purpose:** Score how strongly job postings signal buying intent, weighted by recency and seniority. **Variables:** {{job_postings}}, {{relevant_functions}}. **Model guidance:** claude-haiku-4-5. **Output:** JSON `{score, relevant_posting_count, evidence, rationale}`.
...
**Purpose:** Merge ICP, signal, and engagement sub-scores into a P1/P2/P3 outreach tier with deterministic tie-breaks. **Variables:** {{icp_fit_score}}, {{signal_strength_score}}, {{engagement_score}}. **Model guidance:** claude-haiku-4-5 (pure arithmetic + two rules). **Output:** JSON `{composite, tier, tie_break_applied}`.
...
**Purpose:** Hard-disqualifier gate to run before spending enrichment credits on a record. **Variables:** {{disqualifiers}}, {{company_name}}, {{company_summary}}, {{industry}}, {{employee_count}}, {{country}}. **Model guidance:** claude-haiku-4-5. **Output:** one line — `DISQUALIFY: <rule> — <evidence>`, `PASS`, or `PASS (unverified: <fields>)`.
...
**Purpose:** Score how closely a job title matches a written buyer persona. **Variables:** {{persona_description}}, {{title}}. **Model guidance:** claude-haiku-4-5. **Output:** JSON `{score, ambiguous, reason}`.
