...
Action shape: `{"kind":"connector","integrationSlug":"anthropic","actionSlug":"instruct","config":{"model":"claude-haiku-4-5","temperature":0.3}}` with the substituted prompt in each record's `prompt` field (see [`../../recipes/outreach-activation.md`](../../recipes/outreach-activation.md) step 5). Models: `claude-haiku-4-5` for cheap/bulk, `claude-sonnet-5` for judgment-heavy — each entry says which. **Temperature applies to the haiku tier only** (extraction and classification want `temperature: 0`); `claude-sonnet-5` rejects non-default sampling parameters (the API returns 400) — when a prompt runs on Sonnet 5, drop `temperature` from `config` entirely and rely on the prompt's own output contract for determinism.
