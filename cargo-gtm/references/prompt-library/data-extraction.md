...
Confidence bands: "confirmed" = the text states it explicitly and currently; "inferred" = several consistent indirect statements and nothing contradicting them; "estimated" = calculated or approximated from partial figures in the text; "unknown" = insufficient, contradictory, or only historical evidence. Below the "inferred" bar, return value null with confidence "unknown" — an unsupported value is worse than a missing one, because it will be scored as if it were real. Never widen the allowed value set; if the true answer is outside it, return null.
